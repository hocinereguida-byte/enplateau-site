...
window.EN_PLATEAU_EDITORIAL_DATA = {
  "version": "2026-05-10-v28-48-angles-landing-par-angle",
  "status": "test",
...
window.EN_PLATEAU_EDITORIAL = (function(data) {
  ...
  function getAngleByCode(code, cycleCode) {
    if (!code) return null;
    var normalized = String(code).trim();
    var cycles = cycleCode ? [getCycle(cycleCode)].filter(Boolean) : (data.cycles || []);
    for (var i = 0; i < cycles.length; i++) {
      var angles = cycles[i] && cycles[i].angles ? cycles[i].angles : [];
      for (var j = 0; j < angles.length; j++) {
        var angle = angles[j];
        if (angle && (angle.code === normalized || angle.crmCode === normalized || angle.programmeCode === normalized)) {
          return angle;
        }
      }
    }
  ...
  function getAnglesByLanding(slug) {
    ...
    if (Array.isArray(config.angleCodes) && config.angleCodes.length) {
      ...
      return getAngles(config.cycleCode).filter(function(angle) {
        return allowed[angle.code] === true || allowed[angle.crmCode] === true;
      });
    }
...
window.EN_PLATEAU_LANDING_RENDERER = (function(data, api) {
  ...
  function calUrl(reading, cta, angle) {
    var source = (reading && reading.calSource) || 'lp-industrie';
    var url = 'https://cal.com/en-plateau/echange-editorial?src=' + encodeURIComponent(source) + '&cta=' + encodeURIComponent(cta);
    if (angle && angle.code) url += '&angle=' + encodeURIComponent(angle.code);
    if (angle && angle.typeLecture) url += '&lecture=' + encodeURIComponent(angle.typeLecture);
    return url;
  }

  function setCtas(reading, angle) {
    [
      ['navCta', 'nav'],
      ['heroCta', 'hero'],
      ['midCta', 'mid-conv'],
      ...
      var el = byId(pair[0]);
      if (el) el.setAttribute('href', calUrl(reading, pair[1], angle));
    });
  }
  ...
  function getLandingItems(slug) {
    ...
    if (!config) return [];
    if (config.aliasOf && data.landingPages[config.aliasOf]) config = data.landingPages[config.aliasOf];
  ...
  function getLandingSlugForReading(readingType) {
    var pages = data.landingPages || {};
    var keys = Object.keys(pages);
    for (var i = 0; i < keys.length; i++) {
      var key = keys[i];
      var cfg = pages[key];
      if (cfg && !cfg.aliasOf && cfg.readingType === readingType) return key;
    }
    return null;
  }

  function resolveLandingConfig(slug) {
    var cfg = data.landingPages && data.landingPages[slug];
    if (cfg && cfg.aliasOf && data.landingPages[cfg.aliasOf]) return data.landingPages[cfg.aliasOf];
    return cfg || null;
  }

  function getUrlAngleCode() {
    if (typeof window === 'undefined') return '';
    var params = new URLSearchParams(window.location.search || '');
    return params.get('angle') || params.get('code') || (document.body && document.body.getAttribute('data-angle')) || '';
  }

  function getAngleItem(config, angle) {
    return {
      angle: angle,
      context: api.getContext(config.cycleCode || 'IND', angle.contextCode),
      conversation: api.getConversation(config.cycleCode || 'IND', angle.conversationCode)
    };
  }
  ...
  function getComplementaryAngles(config, angle) {
    ...
    all.forEach(function(candidate) {
      var stable = candidate.code || candidate.crmCode;
      if (stable) byCode[stable] = candidate;
    });
    ...
    if (angle.complementaryCodes && angle.complementaryCodes.length) {
      return angle.complementaryCodes.map(function(code) {
        ...
        return candidate &&
          ...
          (candidate.code || candidate.crmCode) !== (angle.code || angle.crmCode);
      });
    }
  ...
  function renderAudience(config, reading) {
    ...
    var panelEl = byId('audiencePanel');
    if (panelEl && copy.panel) {
      panelEl.style.display = '';
      panelEl.innerHTML = paragraph(copy.panel);
    }
  }
  ...
  function renderAngleNavItem(item, index) {
    ...
    var target = 'angle-' + safeId(angle.code || angle.crmCode || (angle.conversationCode + '-' + angle.contextCode + '-' + index));
  ...
  function renderAnglePanel(item, index, config, reading) {
    ...
    var active = index === 0 ? ' is-active' : '';
    var panelId = 'angle-' + safeId(angle.code || angle.crmCode || (angle.conversationCode + '-' + angle.contextCode + '-' + index));
    var cnum = String(conversation.code || angle.conversationCode || 'C1').replace('C', '');
    ...
    return '<div class="conv-panel' + active + '" id="' + esc(panelId) + '">' +
      ...
      '<div class="finance-angle">' +
        ...
        '<a href="' + esc(calUrl(reading, 'angle', angle)) + '" class="btn-primary angle-cta">Qualifier cet angle en échange éditorial</a>' +
      '</div>' +
    '</div>';
  }
  ...
  function renderAngleHero(config, reading, cycle, item) {
    var angle = item.angle;
    var context = item.context || {};
    var conversation = item.conversation || {};
    var long = angle.formatLongIntervenant || {};

    document.title = (getAngleTitle(angle) || 'Angle Industrie') + ' | En Plateau';
    var titleEl = byId('pageTitle');
    if (titleEl) titleEl.textContent = document.title;
    var meta = document.querySelector('meta[name="description"]');
    if (meta) meta.setAttribute('content', 'Angle En Plateau — ' + getReadingLabel(angle.typeLecture) + ' · ' + (context.label || '') + ' · ' + getAngleTitle(angle));

    setText('navCycle', (cycle && cycle.label ? cycle.label : 'Industrie') + ' · ' + (reading.shortLabel || reading.label || ''));
    setText('heroKicker', 'Cycle ' + ((cycle && cycle.label) || 'Industrie') + ' · ' + (context.label || ''));

    // Reading label + angle title in hero
    setText('heroAngleReading', reading.shortLabel || reading.label || '');
    setText('hero-angle-title', getAngleTitle(angle));

    // Meta: lecture attendue + profils
    var profiles = byId('heroMeta');
    if (profiles) {
      var profileText = (angle.primaryProfiles || []).slice(0, 6).join(' · ');
      profiles.innerHTML =
        '<div class="hero-meta-item"><strong>Lecture attendue</strong>' + esc(reading.label || getReadingLabel(angle.typeLecture)) + '</div>' +
        '<div class="hero-meta-item"><strong>Profils concernés</strong>' + esc(profileText) + '</div>';
    }

    setText('heroCardTitle', 'Votre lecture a-t-elle sa place ici ?');
    setText('heroCardText', 'En 15 minutes, nous vérifions si cet angle correspond à votre position — sans exposer de dossier interne.');
  }

  function renderAngleRarityBlock(reading, item) {
    var target = byId('editorialRarityBlock');
    if (!target) return;
    var angle = item.angle;
    var context = item.context || {};
    ...
    target.innerHTML =
      '<div class="landing-rarete-block">' +
        '<div class="landing-rarete-kicker">Angle proposé</div>' +
        '<h3 class="landing-rarete-title">Votre lecture est sollicitée sur un angle précis.</h3>' +
        '<p class="landing-rarete-text">Cet angle appartient à la conversation « ' + esc(conversation.title || '') + ' », dans le contexte « ' + esc(context.label || '') + ' ». Il est associé à une ' + esc(readingName(reading)) + ' et reste soumis à qualification éditoriale.</p>' +
      '</div>';
  }

  function renderConversationsForAngle(config, reading, item) {
    var angle = item.angle;
    setText('conversations-titre', 'L’angle proposé pour votre contribution');
    setText('conversationsIntro', 'Vous avez été orienté vers cet angle parce que votre fonction, votre organisation ou votre expérience peut aider à rendre lisible un mécanisme précis. Les lectures complémentaires affichées servent uniquement à situer la mise en regard éditoriale.');
    setHtml('conversationsPanel', '');

    var nav = byId('convNav');
    if (nav) nav.innerHTML = renderAngleNavItem(item, 0);

    var content = byId('convContent');
    if (content) content.innerHTML = renderAnglePanel(item, 0, config, reading);

    attachConversationSwitch();
  }

  function renderAngleCta(reading, item) {
    var angle = item.angle;
    setText('cta-titre', 'Qualifier cet angle en échange éditorial');
    setText('ctaText', 'Un échange de 15 minutes permet de vérifier si votre lecture peut prendre place sur cet angle, sans exposer de dossier interne, client, site ou décision confidentielle.');
    setText('ctaDeadline', 'Prochain comité éditorial : 15 juin 2026 · Les échanges doivent être engagés en amont');
    setText('ctaFootnote', 'Angle ' + (angle.code || '') + ' · ' + (reading.label || getReadingLabel(angle.typeLecture)) + ' · Page non indexée');
  }

  function renderAngleLanding(angleCode) {
    var angle = api.getAngleByCode(angleCode, 'IND');
    if (!angle) { console.warn('Angle introuvable :', angleCode); return false; }

    var reading = data.readingTypes[angle.typeLecture] || {};
    var slug = getLandingSlugForReading(angle.typeLecture) || (reading && reading.slug) || 'strategie';
    var config = resolveLandingConfig(slug) || { cycleCode: 'IND', readingType: angle.typeLecture, copy: {} };
    var cycle = api.getCycle(config.cycleCode || 'IND');
    var item = getAngleItem(config, angle);
    var copy = config.copy || {};
    var long = angle.formatLongIntervenant || {};
    var context = item.context || {};
    var conversation = item.conversation || {};
    ...
    // ── Hero
    renderAngleHero(config, reading, cycle, item);
    setCtas(reading, angle);

    // ── Gains / bénéfices (hover cards)
    var vs = config.valueSection;
    if (vs) {
      setText('gains-titre', vs.title || '');
      setText('gainsIntro', vs.intro || '');
      var grid = byId('gainsGrid');
      if (grid) {
        grid.innerHTML = (vs.cards || []).map(function(card, i) {
          return '<div class="gain-card">' +
            '<div class="gain-card__num">0' + (i+1) + '</div>' +
            '<div class="gain-card__title">' + esc(card.title) + '</div>' +
            '<div class="gain-card__label">' + esc(card.label) + '</div>' +
            '<div class="gain-card__body">' + esc(card.text) + '</div>' +
          '</div>';
        }).join('');
      }
    }
    ...
    // ── Angle section
    var kicker = byId('angleKicker');
    if (kicker) kicker.innerHTML = esc(context.label || '') + (conversation.title ? ' &nbsp;·&nbsp; ' + esc(conversation.title) : '');
    setText('angleReading', reading.shortLabel || reading.label || '');
    setText('angle-titre', getAngleTitle(angle));
    setText('angleBody', angle.introMecanisme || angle.questionActivation || '');

    // Bascule + tension
    var bascule = byId('angleBascule');
    if (bascule && (angle.pointBascule || angle.tensionArbitrage)) {
      bascule.innerHTML =
        '<div class="bascule-cell"><div class="bascule-cell__label">Point de bascule</div><p class="bascule-cell__text">' + esc(angle.pointBascule || '') + '</p></div>' +
        '<div class="bascule-cell"><div class="bascule-cell__label">Tension / arbitrage</div><p class="bascule-cell__text">' + esc(angle.tensionArbitrage || '') + '</p></div>';
    }

    // Profiles
    var profEl = byId('angleProfiles');
    if (profEl) {
      var pText = (angle.primaryProfiles || []).join(' · ');
      profEl.innerHTML = '<strong>Profils pressentis&nbsp;:</strong> ' + esc(pText);
    }
    setText('angleSecurite', long.securisation || 'L'échange ne suppose pas de commenter un site, un incident, un client, un fournisseur ou une situation confidentielle. Il s'agit de rendre lisible un mécanisme à partir d'une expérience située.');

    // Angle side CTA
    setText('angleSideCtaTitle', 'Qualifier cet angle en échange éditorial');
    setText('angleSideCtaText', 'En 15 minutes, nous vérifions si votre lecture peut prendre place sur cet angle.');

    // Rarity side
    var rarityEl = byId('angleRarity');
    if (rarityEl) {
      var rarKicker = copy.rarityBlock && copy.rarityBlock.kicker || 'Disponibilité';
      var rarTitle  = copy.rarityBlock && copy.rarityBlock.title || (copy.positionsCountNote || '');
      var rarText   = copy.rarityBlock && copy.rarityBlock.text  || 'Les positions se ferment au fil de la composition éditoriale.';
      rarityEl.innerHTML =
        '<div class="angle-side-rarity__kicker">' + esc(rarKicker) + '</div>' +
        '<p><strong>' + esc(rarTitle) + '</strong></p>' +
        (rarText ? '<p style="margin-top:8px">' + esc(rarText) + '</p>' : '');
    }
    ...
    // ── CTA final
    setText('cta-titre', 'Qualifier cet angle en échange éditorial');
    setText('ctaText', 'Un échange de 15 minutes — sans exposer de chiffre interne, sans engagement — pour vérifier si votre angle a sa place.');
    setText('ctaDeadline', 'Comité éditorial : 15 juin 2026 · Les échanges précèdent la composition finale');
    setText('ctaFootnote', 'Angle ' + (angle.code || '') + ' · ' + (reading.label || getReadingLabel(angle.typeLecture)) + ' · Page non indexée');
  ...
  function autoRender() {
    var angleCode = getUrlAngleCode();
    if (angleCode && renderAngleLanding(angleCode)) return;
  ...
  return {
    ...
    renderAngleLanding: renderAngleLanding
  };
