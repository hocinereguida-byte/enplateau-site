...
(function () {
  ...
  function getReadingType(readingLabel) {
    ...
    const MAP = {
      "strategique": "STRATEGIQUE", "financiere": "FINANCIERE",
      "juridique": "JURIDIQUE", "juridique / reglementaire": "JURIDIQUE",
      "operationnelle": "OPERATIONNELLE",
      "rh / competences": "RH_COMPETENCES", "rh_competences": "RH_COMPETENCES",
      "technologique / systemes": "TECHNOLOGIQUE", "technologique": "TECHNOLOGIQUE",
      "territoriale": "TERRITORIALE",
      "energie / ressources / decarbonation": "ENERGIE_RESSOURCES",
  ...
  function getValueCards(landingPage, reading, actorType, personRole) {
    // Priorité 1 : gainsParProfilV2[groupe][persona] depuis data V67
    const rtData     = getReadingType(reading?.label || reading?.code || "");
    const groupe     = detectProfilGroupe(actorType);
    const personaKey = detectPersonaType(personRole, actorType);
  ...
  function render(bundle) {
    ...
    const valueCards   = getValueCards(landingPage, reading, actorType, personRole);
