/*
  ...
  Version V65.11 — Option A : gains et FAQ par groupe (industriel/éclaireur) × persona (data V67)
  Patch sur V65.10 :
  - detectPersonaType() : détecte DG/DAF/DRH/DIR_JURIDIQUE/DIR_INDUSTRIEL depuis person.role
  - detectProfilGroupe() : détecte industriel vs éclaireur (conseil/avocat/ingenierie)
  - getValueCards()     : lecture depuis gainsParProfilV2[groupe][persona] (data V67)
                          fallback gainsParProfilV2[groupe].default → gainsParProfil V66
  - getFAQ()            : universelles + faqV2[groupe][persona] (data V67)
                          5-6 questions toujours garanties
  - getGainsItems()     : mode items (3 gainsItems typés) ou narratif selon contexte
  Aucune autre modification de V65.10/V65.9.
...
(function () {
  ...
  /* ─────────────────────────────────────────────────────────
     ACCÈS READINGTYPE — V65.11
  ───────────────────────────────────────────────────────── */
  function getReadingType(readingLabel) {
    const rt = DATA.readingTypes || {};
    if (!readingLabel) return null;
    const MAP = {
      "stratégique": "STRATEGIQUE", "financière": "FINANCIERE", "financiere": "FINANCIERE",
      "juridique": "JURIDIQUE", "juridique / réglementaire": "JURIDIQUE",
      "opérationnelle": "OPERATIONNELLE", "operationnelle": "OPERATIONNELLE",
      "rh / compétences": "RH_COMPETENCES", "rh / competences": "RH_COMPETENCES",
      "rh_competences": "RH_COMPETENCES", "technologique / systèmes": "TECHNOLOGIQUE",
      "technologique": "TECHNOLOGIQUE", "territoriale": "TERRITORIALE",
      "énergie / ressources / décarbonation": "ENERGIE_RESSOURCES",
      "energie / ressources": "ENERGIE_RESSOURCES", "energie_ressources": "ENERGIE_RESSOURCES",
    };
    const key = norm(readingLabel);
    const mapped = MAP[key];
    if (mapped && rt[mapped]) return rt[mapped];
    for (const [pattern, rtKey] of Object.entries(MAP)) {
      if (key.includes(norm(pattern).split(" ")[0]) && rt[rtKey]) return rt[rtKey];
    }
    return null;
  }
  ...
  /* ─────────────────────────────────────────────────────────
     GAINS — V65.11 : lit gainsParProfilV2[groupe][persona]
  ───────────────────────────────────────────────────────── */
  function getValueCards(landingPage, reading, actorType, readingLabel, personRole) {
    ...
    if (gainsV2?.[groupe]) {
      ...
      if (personaData) {
        ...
        return [
          { label: orgLabel,  title: personaData.gain,  text: personaData.detail,  gainsItems: personaData.gainsItems || [] },
          { label: funcLabel, title: defaultData.gain,   text: defaultData.detail,  gainsItems: (defaultData.gainsItems || []).slice(2, 4) },
          { label: "Pour vous", title: "Construire une trace professionnelle crédible et réutilisable.",
            ...
            gainsItems: (personaData.gainsItems || []).filter(i => i.type === "réutilisation" || i.type === "différenciation") }
        ];
      }
    }

    // Fallback V66 gainsParProfil plat
    ...
    return [
      { label: actorType === "cabinet_conseil" ? "Pour votre expertise" : "Pour votre organisation",
        title: actorType === "cabinet_conseil" ? "Installer une parole d'autorité sans vendre directement une offre." : "Valoriser une capacité à rendre lisibles les conditions réelles d'une trajectoire industrielle.",
        text: actorType === "cabinet_conseil" ? "La contribution permet de formuler une lecture utile sans présenter une mission." : "La contribution permet à l'organisation de montrer sa capacité de lecture sans exposer de données internes.", gainsItems: [] },
      { label: "Pour votre fonction", title: "Faire reconnaître ce que votre fonction permet de voir.",
        text: "La prise de parole rend visible une capacité de pilotage, de coordination ou d'arbitrage souvent difficile à expliquer.", gainsItems: [] },
      { label: "Pour vous", title: "Construire une trace professionnelle crédible.",
        text: "L'intervention devient un actif éditorial préparé, durable et réutilisable.", gainsItems: [] }
    ];
  }

  /* ─────────────────────────────────────────────────────────
     FAQ — V65.11 : universelles + faqV2[groupe][persona]
  ───────────────────────────────────────────────────────── */
  const FAQ_UNIVERSELLES_FALLBACK = [
    ...
    { question: "Les équipes communication ou juridiques peuvent-elles être associées ?", answer: "Oui. Le périmètre de parole peut être travaillé en amont avec vos équipes communication, affaires publiques ou juridiques si nécessaire." }
  ];
  ...
  function getFAQ(readingLabel, actorType, personRole, landingPage) {
    ...
    const groupe     = detectProfilGroupe(actorType);
    const personaKey = detectPersonaType(personRole, actorType);
    const universelles = toArray(rtData?.faqV2?.universelles?.length ? rtData.faqV2.universelles : FAQ_UNIVERSELLES_FALLBACK);
    const parPersona   = toArray(rtData?.faqV2?.[groupe]?.[personaKey] || rtData?.faqV2?.[groupe]?.default || []);
      ...
      <section class="landing-section landing-section--dark" id="experience-position">
        <div class="landing-container">
          ...
          <div class="landing-grid landing-grid--3">
            ...
            ${card("Ce que cela produit", "Une contribution reconnaissable par les bons interlocuteurs", `Une position claire, crédible et mobilisable dans la durée, depuis une position ${safe(soften(reading))}, auprès des acteurs qui rencontrent les mêmes arbitrages.`, true)}
          </div>
        </div>
      </section>`;
  }
  ...
  function render(bundle) {
    ...
    const valueCards   = getValueCards(landingPage, reading, actorType, readingLabel, personRole);
    const guarantees   = getGuarantees(publicAngle, landingPage, personaFit);
    const processSteps = getProcessSteps(landingPage);

    // V65.11 — FAQ depuis faqV2[groupe][persona] (data V67)
      ...
      <section class="landing-section landing-final">
        <div class="landing-container">
          ...
          <p class="landing-reassurance">
            15 minutes · sans engagement · aucune préparation nécessaire · page privée non indexée
          </p>
        </div>
      </section>
    `;
