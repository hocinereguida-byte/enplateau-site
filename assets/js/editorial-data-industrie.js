...
window.EN_PLATEAU_LANDING_RENDERER = (function(data, api) {
  ...
  function renderAngleHero(config, reading, cycle, item) {
    ...
    if (meta) {
      meta.setAttribute('content', 'Angle En Plateau — ' + getReadingLabel(angle.typeLecture) + ' · ' + (context.label || '') + ' · ' + getAngleTitle(angle));
    }

    setText('navCycle', (cycle && cycle.label ? cycle.label : 'Industrie') + ' · ' + (reading.shortLabel || reading.label || 'Lecture'));
    setText('heroKicker', 'Cycle ' + ((cycle && cycle.label) || 'Industrie') + ' · ' + (context.label || ''));
    setHtml('hero-title', '<em>' + esc(reading.shortLabel || reading.label || getReadingLabel(angle.typeLecture)) + '</em><br>' + esc(getAngleTitle(angle)));
    setText('heroLead', angle.questionActivation || angle.introMecanisme || reading.positioning || '');
    setText('positionsCountNote', 'Angle proposé · ' + (angle.code || '') + ' · ' + (conversation.title || ''));

    var profiles = byId('heroProfiles');
    if (profiles) {
      var profileText = (angle.primaryProfiles || []).slice(0, 6).join(' · ');
      profiles.innerHTML =
        '<div class="hero-profile"><div class="hero-profile-label">Lecture attendue</div><p>' + esc(reading.label || getReadingLabel(angle.typeLecture)) + '</p></div>' +
        '<div class="hero-profile"><div class="hero-profile-label">Profils concernés</div><p>' + esc(profileText || 'Intervenant identifié pour cet angle') + '</p></div>';
    }

    setHtml('heroNote', esc(long.securisation || 'Aucun dossier interne, client, site ou décision confidentielle n’est attendu. L’échange vise à qualifier votre lecture du mécanisme.'));
    setText('heroCardTitle', 'Qualifier cet angle en échange éditorial');
    setText('heroCardText', 'En 15 minutes, nous vérifions si cet angle correspond à votre position, à votre niveau de lecture et aux conditions d’une contribution publiquement tenable.');
  }
  ...
  function renderAngleLanding(angleCode) {
    ...
    if (!angle) {
      console.warn('Angle introuvable :', angleCode);
      return false;
    }
    ...
    // Value section: use landing page config (has valueSection), fallback to reading-level
    var valueConfig = config.valueSection ? config : { valueSection: null };
    renderValueSection(valueConfig);
    renderAngleRarityBlock(reading, item);
    renderAudience(config, reading);
    renderConversationsForAngle(config, reading, item);

    setHtml('dimensions-titre', copy.preparation && copy.preparation.titleHtml || 'Transformer une expérience en position claire, située et publiquement tenable.');
    setText('dimensionsIntro', copy.preparation && copy.preparation.intro || 'La préparation éditoriale part de votre lecture de l’angle, puis la structure pour éviter une parole trop générale, intrusive ou surexposée.');
    renderPreparationDimensions(config);
    renderProcess(config);
    renderGuarantees(config);
    renderFaq(config);
    renderAngleCta(reading, item);
